...
const ResultsDisplay = ({ results }) => {
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.href = image;
      link.download = `Il_Mio_TDEE_Report.png`;
      link.click();
    } catch (err) {
  ...
  return (
    <Box>
      <Box 
        ref={resultsRef} 
        className={downloading ? 'export-mode' : ''}
        sx={{ 
          mt: 2, 
          textAlign: 'center', 
          p: { xs: 1, md: 3 }, 
          ...
          '&.export-mode .hide-on-export': {
            display: 'none !important'
          },
          '&.export-mode': {
            background: 'linear-gradient(135deg, #0a0515 0%, #17102e 100%)',
            pb: 4
          }
        }}
      >
      ...
      <Card sx={{ mb: 3 }} className="hide-on-export">
        <CardContent>
          <Typography variant="h6" color="primary" gutterBottom>
            Dettaglio Componenti TDEE
          </Typography>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <Typography variant="h5" sx={{ color: options[selectedOption].color, mb: 3, fontWeight: 'bold' }}>
            Macronutrienti per {options[selectedOption].label} ({options[selectedOption].calories} kcal)
          </Typography>
        ...
        <Grid size={{ xs: 12}}>
          ...
          <Typography className="hide-on-export" variant="body2" color="text.secondary" sx={{ mt: 3, textAlign: 'left' }}>
            <strong>Nota importante:</strong> Quando cambi l'obiettivo calorico, i grammi di proteine e grassi rimangono invariati. 
            Solo i carboidrati vengono aggiustati per raggiungere il nuovo target calorico. Questo approccio è ideale perché 
            mantiene costante l'apporto proteico per preservare la massa muscolare e mantiene un adeguato apporto di grassi 
      ...
      <Box className="hide-on-export" sx={{ mt: 3, mb: 2 }}>
        <Typography variant="body2" color="text.secondary">
      ...
      <Button 
        ...
        sx={{ 
          mt: 2, 
          py: 1.5, 
          background: 'linear-gradient(45deg, #00d4ff 10%, #8b5cf6 90%)',
      ...
      >
        {downloading ? <CircularProgress size={24} color="inherit" /> : 'Scarica il Report come Immagine'}
      </Button>
    </Box>
  );
};
